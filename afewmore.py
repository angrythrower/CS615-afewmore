...
class SecurityGroups:
    def __init__(self, sgroup):
        self.gname = sgroup["GroupName"]
        self.gid = sgroup["GroupId"]
    def __str__(self):
...
class Instance:
    def __init__(self, instance_id):
        ...
        if out:
            ...
            for group in instance["SecurityGroups"]:
                self.sgroups.append(SecurityGroups(group))
        else:
            elog(err)
    ...
    # will check if the instance is available to log in with command ssh-keyscan. 
    ...
    def isReady(self):
        ...
        if out :
            ...
            if err:
                elog(err)
            log("instance: {0} is ready, hosts added".format(self.insId))
            return True
        elif err :
            elog(err)

        log("instance: {0} is not ready".format(self.insId))
        return False
